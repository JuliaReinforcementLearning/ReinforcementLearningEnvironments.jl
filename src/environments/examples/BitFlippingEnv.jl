...
"""
In Bit Flipping Environment we have n bits. The actions are 1 to n where executing
i-th action flips the i-th bit of the state. For every episode we sample uniformly and inital
state as well as the target state.
...
RLBase.state(env::BitFlippingEnv) = env.state
RLBase.state(env::BitFlippingEnv, ::GoalState) = env.goal_state
RLBase.state_space(env::BitFlippingEnv) = Space(fill(false..true,env.N))
